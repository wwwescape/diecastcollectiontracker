...
app.delete("/api/purge", async (req, res) => {
  try {
    ...
    // Delete all images from the uploads folder
    const uploadsDir = path.join(__dirname, "uploads");
    ...
    res.json({ message: "All data and images have been purged successfully" });
  } catch (error) {
    console.error("Error purging data:", error);
    res.status(500).json({ message: "Error purging data" });
  }
});
